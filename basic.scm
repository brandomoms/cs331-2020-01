...
; Code from 4/15 - Scheme: Basics


(display "This file contains sample code from April 15, 2020,")
(newline)
...
(define (mymap f xs)
  (cond
    [(null? xs)        null]
    [(not (pair? xs))  (error "mymap: arg #2 has bad type")]
    [else              (cons
                        (f (car xs))
                        (mymap f (cdr xs))
                        )]
...
(define (myfilter p xs)
  (cond
    [(null? xs)        null]
    [(not (pair? xs))  (error "myfilter: arg #2 has bad type")]
    [else
     (let (
           [first  (car xs)]
