...
-- REPL/Shell for Degu Programming Language
...
local degustate  -- Degu variable values
...
-- ***** Functions for Degu REPL *****


-- printHelp
-- Print help for REPL.
local function printHelp()
    io.write("Type Degu code to execute it.\n")
    io.write("Commands (these may be abbreviated;")
    io.write(" for example, \":e\" for \":exit\")\n")
    io.write("  :exit          - Exit.\n")
    io.write("  :run FILENAME  - Execute Degu source file.\n")
    io.write("  :clear         - Clear Degu state.\n")
...
-- clearState
-- Clear Degu state: functions, simple variables, arrays.
...
-- runDegu
-- Given a string, attempt to treat it as source code for a Degu
-- program, and execute it. I/O uses standard input & output.
--
-- Parameters:
--   program  - Degu source code
--   state    - Values of Degu variables as in interpit.interp.
...
function runDegu(program, state, execmsg)
...
-- runFile
-- Given filename, attempt to read source for a Degu program from it,
...
function runFile(fname, printmsg)
    ...
    if not readable(fname) then
        errMsg("Degu source file not readable: '"..fname.."'")
        return
    end
    local source = ""
    ...
    good, done, degustate = runDegu(source, degustate, execmsg)
    if not (good and done) then
        errMsg("Syntax error in Degu source file: '"..fname.."'")
    end
end
...
-- repl
-- Degu REPL. Prompt & get a line. If it is blank, then exit. If it
-- looks like the filename of a Degu source file, then get Degu
-- source from it, execute, and exit. Otherwise, treat line as Degu
-- program, and attempt to execute it. If it looks like an incomplete
-- Degu program, then keep inputting, and continue to attempt to
-- execute. REPEAT.
function repl()
    ...
    while true do
        ...
        if line:sub(1,1) == ":" then    -- Command
            ...
            source = source .. line
            good, done, degustate = runDegu(source, degustate)
            if (not good) and done then
...
-- Initialize Degu state
clearState()

-- Command-line argument? If so treat as Degu source filename, read
-- source, and execute.
if arg[1] ~= nil then
    ...
    io.flush()  -- Ensure previous output is done before input
    io.read("*l")
-- Otherwise, fire up the Degu REPL.
else
    repl()
end
