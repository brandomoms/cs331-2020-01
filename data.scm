...
; Code from 4/17 - Scheme: Data


(display "This file contains sample code from April 17, 2020,")
(newline)
...
; forcemaybe
; Given an item. If is not a promise, returns it. If it is a promise,
; forces it and returns the result.
(define (forcemaybe x)
  (cond
    [(promise? x)   (force x)]
    [else           x]
...
(define (take count lxs)
  (cond
    [(<= count 0)    null]
    [(null? lxs)     null]
    [(pair? lxs)     (cons (forcemaybe (car lxs))
                           (take (- count 1) (forcemaybe (cdr lxs)))
                           )
                     ]
